...
public class RentManager {
    ...
    public void setForRent(Player player, String landName, int amount, int days, boolean repeatable, boolean resadmin) {
        ...
        if (Residence.getTransactionManager().isForSale(landName)) {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("SellRentFail"));
            return;
        }
        ...
        if (!resadmin) {
            ...
            if (this.getRentableCount(player.getName()) >= group.getMaxRentables()) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("ResidenceMaxRent"));
                return;
            }
        }
        if (!rentableLand.containsKey(landName)) {
            ...
            if (split.length != 0) {
                player.sendMessage(TextFormat.GREEN + Residence.getLanguage().getPhrase("ResidenceForRentSuccess", TextFormat.YELLOW + split[split.length - 1] + TextFormat.GREEN + "." + TextFormat.YELLOW + amount + TextFormat.GREEN + "." + TextFormat.YELLOW + days + TextFormat.GREEN));
            }
        } else {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("ResidenceAlreadyRent"));
        }
    }

    public void rent(Player player, String landName, boolean repeat, boolean resadmin) {
        if (!Residence.getConfigManager().enabledRentSystem()) {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("RentDisabled"));
            return;
        }
        ClaimedResidence res = Residence.getResidenceManager().getByName(landName);
        if (res != null) {
            if (res.getPermissions().getOwner().equalsIgnoreCase(player.getName())) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("OwnerRentFail"));
                return;
            }
        } else {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("InvalidResidence"));
        ...
        if (!this.isForRent(landName)) {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("ResidenceNotForRent"));
            return;
        }
        if (this.isRented(landName)) {
            String[] split = landName.split("\\.");
            if (split.length != 0) {
                player.sendMessage(Residence.getLanguage().getPhrase("ResidenceAlreadyRented", TextFormat.YELLOW + split[split.length - 1] + TextFormat.RED + "." + TextFormat.YELLOW + this.getRentingPlayer(landName)));
            }
            return;
        }
        ...
        if (Residence.getEconomyManager().canAfford(player.getName(), land.cost)) {
            ...
            if (Residence.getEconomyManager().transfer(player.getName(), res.getPermissions().getOwner(), land.cost)) {
                ...
                String[] split = landName.split("\\.");
                if (split.length != 0) {
                    player.sendMessage(TextFormat.GREEN + Residence.getLanguage().getPhrase("ResidenceRentSuccess", TextFormat.YELLOW + split[split.length - 1] + TextFormat.GREEN + "." + TextFormat.YELLOW + land.days + TextFormat.GREEN));
                }
            } else {
                player.sendMessage(TextFormat.RED + "Error, unable to transfer money...");
            }
        } else {
    ...
    public void removeFromForRent(Player player, String landName, boolean resadmin) {
        ...
        if (rent == null) {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("ResidenceNotRented"));
            return;
        }
        if (resadmin || rent.player.equalsIgnoreCase(player.getName())) {
            ...
            player.sendMessage(TextFormat.GREEN + Residence.getLanguage().getPhrase("ResidenceUnrent", TextFormat.YELLOW + landName + TextFormat.GREEN));
        } else {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("NoPermission"));
        }
    }
    ...
    public void unrent(Player player, String landName, boolean resadmin) {
        ...
        ClaimedResidence res = Residence.getResidenceManager().getByName(landName);
        if (res == null) {
            player.sendMessage(TextFormat.YELLOW + Residence.getLanguage().getPhrase("InvalidResidence"));
            return;
        }
        ...
        if (rentedLand.containsKey(landName) && !resadmin) {
            if (split.length != 0) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("ResidenceAlreadyRented", TextFormat.YELLOW + split[split.length - 1] + TextFormat.RED + "." + TextFormat.YELLOW + rentedLand.get(landName).player) + TextFormat.YELLOW);
            }
            return;
        }
        if (rentableLand.containsKey(landName)) {
            ...
            if (split.length != 0) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("ResidenceRemoveRentable", TextFormat.YELLOW + split[split.length - 1] + TextFormat.RED));
            }
        ...
        while (it.hasNext()) {
            ...
            if (land.endTime <= System.currentTimeMillis()) {
                ...
                if (Residence.getConfigManager().debugEnabled()) {
                    MainLogger.getLogger().info("Rent Check: " + next.getKey());
                }
                if (res != null) {
    ...
    public void setRentRepeatable(Player player, String landName, boolean value, boolean resadmin) {
        ...
        if (land != null && res != null && (res.getPermissions().getOwner().equalsIgnoreCase(player.getName()) || resadmin)) {
            ...
            if (value && split.length != 0) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("RentableEnableRenew", TextFormat.YELLOW + split[split.length - 1] + TextFormat.RED));
            } else if (split.length != 0) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("RentableDisableRenew", TextFormat.YELLOW + split[split.length - 1] + TextFormat.RED));
            }
        }
    }
    ...
    public void setRentedRepeatable(Player player, String landName, boolean value, boolean resadmin) {
        ...
        if (land != null && (land.player.equals(player.getName()) || resadmin)) {
            ...
            if (value && split.length != 0) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("RentEnableRenew", TextFormat.YELLOW + split[split.length - 1] + TextFormat.RED));
            } else if (split.length != 0) {
                player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("RentDisableRenew", TextFormat.YELLOW + split[split.length - 1] + TextFormat.RED));
            }
        }
    }
    ...
    public void printRentInfo(Player player, String landName) {
        ...
        if (rentable != null) {
            player.sendMessage(TextFormat.GOLD + Residence.getLanguage().getPhrase("Land") + ":" + TextFormat.DARK_GREEN + landName);
            player.sendMessage(TextFormat.YELLOW + Residence.getLanguage().getPhrase("Cost") + ": " + TextFormat.DARK_AQUA + rentable.cost + " per " + rentable.days + " days");
            player.sendMessage(TextFormat.GREEN + Residence.getLanguage().getPhrase("RentableAutoRenew") + ":" + TextFormat.DARK_AQUA + rentable.repeatable);
            if (rented != null) {
                player.sendMessage(TextFormat.GOLD + Residence.getLanguage().getPhrase("Status") + ":" + TextFormat.YELLOW + " " + Residence.getLanguage().getPhrase("ResidenceRentedBy", TextFormat.RED + rented.player + TextFormat.YELLOW));
                player.sendMessage(TextFormat.YELLOW + Residence.getLanguage().getPhrase("RentExpire") + ":" + TextFormat.GREEN + new Date(rented.endTime));
                player.sendMessage(TextFormat.GREEN + Residence.getLanguage().getPhrase("RentAutoRenew") + ":" + TextFormat.DARK_AQUA + rented.autoRefresh);
            } else {
                player.sendMessage(TextFormat.GOLD + Residence.getLanguage().getPhrase("Status") + ":" + TextFormat.GREEN + " " + Residence.getLanguage().getPhrase("Available"));
            }
        } else {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("ResidenceNotForRent"));
        }
    ...
    public void printRentableResidences(Player player) {
        ...
        player.sendMessage(TextFormat.YELLOW + Residence.getLanguage().getPhrase("RentableLand") + ":");
