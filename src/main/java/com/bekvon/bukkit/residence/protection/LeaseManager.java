...
public class LeaseManager {
    ...
    public void setExpireTime(Player player, String area, int days) {
        ...
        if (manager.getByName(area) != null) {
            ...
            if (player != null) {
                player.sendMessage(TextFormat.GREEN + Residence.getLanguage().getPhrase("LeaseRenew", getExpireTime(area).toString()));
            }
        } else {
            if (player != null) {
    ...
    public void renewArea(String area, Player player) {
        if (!leaseExpires(area)) {
            player.sendMessage(TextFormat.RED + Residence.getLanguage().getPhrase("LeaseNotExpire"));
            return;
        }
        ...
        if (rem + add > max) {
            ...
            player.sendMessage(TextFormat.GOLD + Residence.getLanguage().getPhrase("LeaseRenewMax"));
            player.sendMessage(TextFormat.YELLOW + Residence.getLanguage().getPhrase("LeaseRenew", TextFormat.GREEN + "" + getExpireTime(area)) + TextFormat.YELLOW);
            return;
        }
        ...
        player.sendMessage(TextFormat.YELLOW + Residence.getLanguage().getPhrase("LeaseRenew", TextFormat.GREEN + "" + getExpireTime(area)));
    }
                    ...
                    if (Residence.getConfigManager().enableEconomy() && Residence.getConfigManager().autoRenewLeases()) {
                        if (cost == 0) {
                            ...
                            if (Residence.getConfigManager().debugEnabled()) {
                                MainLogger.getLogger().info("Lease Renewed From Residence Bank: " + resname);
                            }
                        } else if (Residence.getEconomyManager().canAfford(owner, cost)) {
                            if (Residence.getEconomyManager().subtract(owner, cost)) {
                                renewed = true;
                                if (Residence.getConfigManager().debugEnabled()) {
                                    MainLogger.getLogger().info("Lease Renewed From Economy: " + resname);
                                }
                            }
                        }
                    }
                    if (!renewed) {
                        if (!Residence.getConfigManager().enabledRentSystem() || !Residence.getRentManager().isRented(resname)) {
                            ...
                            if (!resevent.isCancelled()) {
                                ...
                                it.remove();
                                if (Residence.getConfigManager().debugEnabled()) {
                                    MainLogger.getLogger().info("Lease NOT removed, Removing: " + resname);
                                }
                            }
                        }
                    } else {
                        ...
                        if (Residence.getConfigManager().debugEnabled()) {
                            MainLogger.getLogger().info("Lease Renew Old: " + next.getValue());
                        }
                        next.setValue(System.currentTimeMillis() + daysToMs(limits.getLeaseGiveTime()));
                        if (Residence.getConfigManager().debugEnabled()) {
                            MainLogger.getLogger().info("Lease Renew New: " + next.getValue());
                        }
                    }
                }
            }
    ...
    public void resetLeases() {
        ...
        MainLogger.getLogger().info("[Residence] - Set default leases.");
    }
